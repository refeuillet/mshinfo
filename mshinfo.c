...
{
  int NmbVer, dim, deg, FilVer, NmbCor, NmbRidg, NmbStr;
  ...
  if (subsol == NULL)
  { //--- no extension sol[b], check if it is a mesh name without extnsion
    if (sub != NULL)
      sol[sub - name] = '\0'; // change file name to be opened as sol even if mesh extension is given
  ...
  if (subsol != NULL)
  { //--- a file containing char ".sol" is given in input
  ...
  { //-- check if a solution with the same name as the mesh one exists
    strcat(sol, ".sol");
    InpSol = GmfOpenMesh(sol, GmfRead, &FilVer, &dim);
    if (InpSol == 0)
    {
